...
//  minilisp
//
//  Created by Fausto Saporito on 06/09/2021.
//  Based on rui314/minilisp github repo.
//
...
enum {
    ...
    TMACRO,
    TENV,
    // The marker that indicates the object has been moved to other
    // location by GC. The new location can be found at the forwarding
    // pointer. Only the functions to do garbage collection set and
    // handle the object of this type. Other functions will never see the
    // object of this type.
    TMOVED,
    // Const objects. They are statically allocated and will never be
    // managed by GC.
    TTRUE,
    TNIL,
    TDOT,
...
// The object type
typedef struct Obj {
    // The first word of the object represents the type of the object.
    // Any code that handles object needs to check its type first,
    // then access the following union members.
    int type;

    // The total size of the object, including "type" field, this field,
    // the contents, and the padding at the end of the object.
    size_t size;
...
// Currently we are using Cheney's copying GC algorithm, with which
// the available memory is split into two halves and all objects are moved
// from one half to another every time GC is invoked. That means the
// address of the object keeps changing. If you take the address of an
// object and keep it in a C variable, dereferencing it could cause SEGV
// because the address becomes invalid after GC runs.
//
// In order to deal with that, all access from C to Lisp objects will
// go through two levels of pointer dereferences. The C local variable
// is pointing to a pointer on the C stack, and the pointer is pointing to
// the Lisp object. GC is aware of the pointers in the stack and updates
// their contents with the objects' new addresses when GC happens.
//
// The following is a macro to reserve the area in the C stack for the
// pointers. The contents of this area are considered to be GC root.
//
// Be careful not to bypass the two levels of pointer indirections.
// If you create a direct pointer to an object, it'll cause a subtle bug.
// Such code would work in most cases but fails with SEGV if
// GC happens during the execution of the code. Any code that allocates
// memory may invoke GC.
...
// Round up the given value to a multiple of size. Size must be a
// power of 2. It adds size - 1 first, then zero-ing the least
// significant bits to make the result a multiple of size. I know
// these bit operations may look a little bit tricky, but it's
// efficient and thus frequently used.
static inline size_t roundup(size_t var, size_t size) {
    return (var + size - 1) & ~(size - 1);
}

// Allocates memory block. This may start GC if we don't have
// enough memory.
static Obj *alloc(void *root, int type, size_t size) {
    // The object must be large enough to contain a pointer for
    // the forwarding pointer. Make it larger if it's smaller
    // than that.
    ...
    // Round up the object size to the nearest alignment boundary,
    // so that the next object will be allocated at the proper
    // alignment boundary. Currently we align the object at the
    // same boundary as the pointer.
    size = roundup(size, sizeof(void *));

    // If the debug flag is on, allocate a new memory space to
    // force all the existing objects to move to new addresses,
    // to invalidate the old addresses. By doing this the GC
    // behavior becomes more predictable and repeatable. If
    // there's a memory bug that the C variable has a direct
    // reference to a Lisp object, the pointer will become
    // invalid by this GC call. Dereferencing that will
    // immediately cause SEGV.
    if (always_gc && !gc_running)
        gc(root);

    // Otherwise, run GC only when the available memory is not
    // large enough.
    if (!always_gc && MEMORY_SIZE < mem_nused + size)
        gc(root);

    // Terminate the program if we couldn't satisfy the memory
    // request. This can happen if the requested size was too
    // large or the from-space was filled with too many live
    // objects.
...
// Cheney's algorithm uses two pointers to keep track of GC status. At
// first both pointers point to the beginning of the to-space. As GC
// progresses, they are moved towards the end of the to-space. The
// objects before "scan1" are the objects that are fully copied. The
// objects between "scan1" and "scan2" have already been copied, but
// may contain pointers to the from-space. "scan2" points to the
// beginning of the free space.
static Obj *scan1;
static Obj *scan2;

// Moves one object from the from-space to the to-space. Returns the
// object's new address. If the object has already been moved, does
// nothing but just returns the new address.
static inline Obj *forward(Obj *obj) {
    // If the object's address is not in the from-space, the object is
    // not managed by GC nor it has already been moved to the to-space.
    ptrdiff_t offset = (uint8_t *)obj - (uint8_t *)from_space;
    if (offset < 0 || MEMORY_SIZE <= offset)
        return obj;

    // The pointer is pointing to the from-space, but the object there
    // was a tombstone. Follow the forwarding pointer to find the new
    ...
    // Put a tombstone at the location where the object used to occupy,
    // so that the following call of forward() can find the object's
    // new location.
    obj->type = TMOVED;
    obj->moved = newloc;
    return newloc;
...
static void gc(void *root) {
    ...
    // Initialize the two pointers for GC. Initially they point to
    // the beginning of the to-space.
    scan1 = scan2 = memory;

    // Copy the GC root objects first. This moves the pointer scan2.
    forward_root_objects(root);

    // Copy the objects referenced by the GC root objects located between
    // scan1 and scan2. Once it's finished, all live objects (i.e. objects
    // reachable from the root) will have been copied to the to-space.
    while (scan1 < scan2) {
        switch (scan1->type) {
        case TLONG:
        case TDOUBLE:
...
static Obj *make_long(void *root, long value) {
    // fprintf(stderr,"MAKELONG\n");
    Obj *r = alloc(root, TLONG, sizeof(long));
    r->value = value;
    return r;
}

static Obj *make_double(void *root, double value) {
...
static Obj *make_num(void *root, double value) {
    if (isdouble == 1) {
        return make_double(root, value);
    } else {
        return make_long(root, (long)value);
    }
}
...
static Obj *make_function(void *root, Obj **env, int type,
                          Obj **params, Obj **body) {
...
static Obj *read_list(void *root, FILE *f) {
    DEFINE3(obj, head, last);
    *head = Nil;
    for (;;) {
        *obj = read_expr(root, f);
        if (!*obj)
            return error("Unclosed parenthesis");
        if (*obj == Cparen)
            return reverse(*head);
        if (*obj == Dot) {
            *last = read_expr(root, f);
            if (read_expr(root, f) != Cparen)
                return error("Closed parenthesis expected after dot");
...
// it will not create a new symbol but return the existing one.
...
// Reader marcro ' (single quote). It reads an expression and
// returns (quote <expr>).
static Obj *read_quote(void *root, FILE *f) {
    DEFINE2(sym, tmp);
    *sym = intern(root, "quote");
    *tmp = read_expr(root, f);
    *tmp = cons(root, tmp, &Nil);
    *tmp = cons(root, sym, tmp);
    return *tmp;
}

static double read_number(int val, FILE *f) {
    double divisor = 10.0;
    char ch;
    double r = (double)val;
    isdouble = 0;
    
    while (isdigit(peek(f)))
        r = r * 10 + (fgetc(f) - '0');
    
    if (peek(f) == '.') {
        isdouble = 1;
        ch = fgetc(f);                 // skip decimal dot
...
static Obj *read_symbol(void *root, char c, FILE *f) {
    char buf[SYMBOL_MAX_LEN + 1];
    buf[0] = c;
    int len = 1;
    while (isalnum(peek(f)) || strchr(symbol_chars, peek(f))) {
        if (SYMBOL_MAX_LEN <= len)
            return error("Symbol name too long");
        buf[len++] = fgetc(f);
    }
    buf[len] = '\0';
...
static Obj *read_expr(void *root, FILE *f) {
    for (;;) {
        ...
        return error("Don't know how to handle %c", c);
    }
}
...
static Obj *push_env(void *root, Obj **env, Obj **vars, Obj **vals) {
    DEFINE3(map, sym, val);
    *map = Nil;
    for (; (*vars)->type == TCELL; *vars = (*vars)->cdr,
         *vals = (*vals)->cdr) {
        if ((*vals)->type != TCELL)
            return error("Cannot apply function: number of argument "
                  "does not match");
        *sym = (*vars)->car;
        *val = (*vals)->car;
...
// Evaluates the list elements from head and returns the last
// return value.
...
// Evaluates all the list elements and returns their return
...
static Obj *apply_func(void *root, Obj **env, Obj **fn,
                       Obj **args) {
...
// Apply fn with args.
static Obj *apply(void *root, Obj **env, Obj **fn,
                  Obj **args) {
    if (!is_list(*args))
...
static Obj *eval(void *root, Obj **env, Obj **obj) {
    ...
    case TSYMBOL: {
        ...
        if (!bind)
            return error("Undefined symbol: %s", (*obj)->name);
        return bind->cdr;
    }
    case TCELL: {
        ...
        *args = (*obj)->cdr;
        if ((*fn)->type != TPRIMITIVE && (*fn)->type != TFUNCTION)
            return error("The head of a list must be a function");
        return apply(root, env, fn, args);
    }
    default:
        return error("Bug: eval: Unknown tag type: %d", (*obj)->type);
    }
}
//======================================================================
// Bignums
...
static Obj *prim_setq(void *root, Obj **env, Obj **list) {
    if (length(*list) != 2 || (*list)->car->type != TSYMBOL)
        return error("Malformed setq");
    DEFINE2(bind, value);
    *bind = find(env, (*list)->car);
    if (!*bind)
        return error("Unbound variable %s", (*list)->car->name);
    *value = (*list)->cdr->car;
...
static Obj *handle_function(void *root, Obj **env,
    ...
    if ((*list)->type != TCELL || !is_list((*list)->car)
        || (*list)->cdr->type != TCELL)
        return error("Malformed lambda");
    Obj *p = (*list)->car;
    for (; p->type == TCELL; p = p->cdr)
        if (p->car->type != TSYMBOL)
            return error("Parameter must be a symbol");
...
static Obj *handle_defun(void *root, Obj **env,
                         Obj **list, int type) {
    if ((*list)->car->type != TSYMBOL || (*list)->cdr->type != TCELL)
        return error("Malformed defun");
    DEFINE3(fn, sym, rest);
    *sym = (*list)->car;
    *rest = (*list)->cdr;
...
static Obj *prim_macroexpand(void *root, Obj **env, Obj **list) {
    if (length(*list) != 1)
        return error("Malformed macroexpand");
    DEFINE1(body);
    *body = (*list)->car;
...
static Obj *prim_num_eq(void *root, Obj **env, Obj **list) {
    ...
    if ((x->type != TLONG && x->type != TDOUBLE && x->type != TBIGN) ||
        (y->type != TLONG && y->type != TDOUBLE && y->type != TBIGN))
        return error("= only takes numbers");
...
static Obj *load_file(void *root, Obj **env, Obj **list) {
    ...
    strcpy(fname, x->str);
    f = fopen(fname,"r");
    if (f == NULL) return error("Cannot open file");
    while ((c=fgetc(f)) != EOF) {
        ungetc(c,f);
        *expr = read_expr(root, f);
        if (!*expr)
            continue;
        if (*expr == Cparen)
            return error("Stray close parenthesis");
        if (*expr == Dot)
            return error("Stray dot");
        if (*expr == Dquote)
            return error("Stray double quote");
...
// Returns true if the environment variable is defined and not the
// empty string.
...
int main(int argc, char **argv) {
    ...
    *env = make_env(root, &Nil, &Nil);
    ...
    // Load library file if present
    f = fopen("/Users/fausap/lib.l","r");
    if (f != NULL) {
        while ((c=fgetc(f)) != EOF) {
            ungetc(c,f);
            *expr = read_expr(root, f);
